...
const foundationProjects = [
  {
    code: "26-055",
    name: "Oakfield Trails — Harvest Club",
    status: "bidding",
  ...
  {
    code: "26-068",
    name: "JHACH Specialty Pharmacy",
    status: "pricing",
    phase: "final pricing",
    owner: "M. Carter",
    startDate: "2026-07-28",
    dueDate: "2026-08-11",
    completionDate: "2027-04-16",
  },
  {
    code: "26-052",
    name: "PHSC East Campus Repairs",
    status: "active",
    phase: "scope review",
    owner: "R. Green",
  ...
  {
    code: "CMAR",
    name: "K-Bar Ranch Park",
    status: "planning",
    phase: "ROM development",
    owner: "A. Foster",
...
export async function ensureFoundationCompany() {
  ...
  const inserted = await db
    .insert(companies)
    .values({ name: "HCO Builders", slug: "hco-builders" })
    .returning();
  return inserted[0];
}
...
export async function updateProject(
  ...
  if (!allowedFields.has(field))
    throw new Response("Unsupported project field", { status: 400 });
  const db = getDatabase();
  const company = await ensureFoundationCompany();
  const normalized = field.endsWith("Date") && !value ? null : value;
  await db
    .update(projects)
    .set({ [field]: normalized, updatedAt: new Date() })
    .where(and(eq(projects.id, id), eq(projects.companyId, company.id)));
}
...
export async function bulkUpdateProjects(ids: string[], status: string) {
  ...
  await db
    ...
    .where(and(eq(projects.companyId, company.id), inArray(projects.id, ids)));
}
...
export async function createProject(input?: Record<string, string>) {
  ...
  const inserted = await db
    ...
    .values({
      ...
      phase: input?.phase?.trim() || "document review",
      ...
      data: {
        ...
        sizeSf: Number(input?.sizeSf || 0),
        anticipatedValue: Number(input?.anticipatedValue || 0),
        estimatedValue: Number(input?.estimatedValue || 0),
        proposedValue: Number(input?.proposedValue || 0),
        customFields,
      },
    })
    .returning();
...
export async function createProjectsFromUpload(file: File) {
  ...
  if (file.name.toLowerCase().endsWith(".json")) {
    ...
    if (!header)
      throw new Response("The uploaded file is empty", { status: 400 });
    const keys = header.split(",").map((value) => value.trim());
    rows = lines.map((line) =>
      Object.fromEntries(
        keys.map((key, index) => [key, line.split(",")[index]?.trim() ?? ""]),
      ),
    );
  }
  const company = await ensureFoundationCompany(),
  ...
  const values = rows
    ...
    .map((row, index) => ({
      companyId: company.id,
      code: String(row.code || `IMPORT-${Date.now()}-${index + 1}`),
      name: String(row.name || "Untitled project"),
      status: String(row.status || "planning"),
      phase: String(row.phase || "preconstruction"),
      ...
      completionDate: row.completionDate ? String(row.completionDate) : null,
    }));
  if (!values.length)
    throw new Response("No project rows were found", { status: 400 });
