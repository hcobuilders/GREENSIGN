...
async function companyId() {
  ...
  const [company]=await db.select().from(companies).where(eq(companies.slug,"hco-builders")).limit(1);
...
export async function listFeedback() {
  ...
  return db.select().from(feedbackItems).where(eq(feedbackItems.companyId,id)).orderBy(desc(feedbackItems.createdAt));
}

export async function createFeedback(note:string,page:string,context:Record<string,unknown>) {
