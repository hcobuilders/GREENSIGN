...
export async function createWorkflowRecord(toolKey:string,type:string,projectId:string|null,payload:Record<string,unknown>={}){const db=getDatabase(),tenant=await company();await db.insert(workflowRecords).values({companyId:tenant.id,projectId:projectId||null,toolKey,type,state:"draft",payload});}
